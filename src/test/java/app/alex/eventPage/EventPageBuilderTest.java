...
/**
 * Comprehensive test suite for EventPageBuilder.
 * Tests builder pattern implementation and dependency injection.
 * Note: GUI creation tests are limited due to headless environment constraints.
 */
class EventPageBuilderTest {

    private EventPageBuilder builder;
    ...
    void setUp() {
        ...
        when(mockDataAccessFactory.getEventDataAccess()).thenReturn(mock(data_access.alex.TodaysEventDataAccessObject.class));
    }
    ...
    void testConstructorWithDataAccessFactory() {
        try (MockedStatic<AppDataAccessFactory> mockedStatic = mockStatic(AppDataAccessFactory.class)) {
            ...
            // When
            builder = new EventPageBuilder(mockDataAccessFactory);
    ...
    @Test
    @DisplayName("Should access data access factory dependencies")
    ...
    @Test
    @DisplayName("Should handle build method call without exceptions in dependency setup")
    void testBuildDependencySetup() {
        try (MockedStatic<AppDataAccessFactory> mockedStatic = mockStatic(AppDataAccessFactory.class);
            ...
            verify(mockDataAccessFactory, atLeastOnce()).getEventDataAccess();
        }
    }
    ...
    void testBuilderInstantiation() {
        try (MockedStatic<AppDataAccessFactory> mockedStatic = mockStatic(AppDataAccessFactory.class)) {
            ...
            EventPageBuilder builder1 = new EventPageBuilder();
            EventPageBuilder builder2 = new EventPageBuilder(mockDataAccessFactory);
